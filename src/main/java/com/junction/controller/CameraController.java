...
public class CameraController {
	...
	private CameraPojo openStream(String ip, String username, String password, String channel, String stream,
		...
		if (null != starttime && !"".equals(starttime)) {
			...
			rtsp = "rtsp://" + username + ":" + password + "@" + IP + ":554/cam/realmonitor?channel=" + channel + "&subtype=0";
//			rtsp ="rtsp://wowzaec2demo.streamlock.net/vod/mp4:BigBuckBunny_115k.mov";
			System.out.println(rtsp+"：rtsp流");
	...
	public Map<String, String> closeCamera(@RequestBody String token) {
		// 返回结果
		Map<String, String> map = new HashMap<String, String>();
	try {
		if (null != token && !"".equals(token)) {
			...
			for (String tokens : tokenArr) {
				if (jobMap.containsKey(tokens) && CacheUtil.STREAMMAP.containsKey(tokens)) {
					if (0 < CacheUtil.STREAMMAP.get(tokens).getCount()) {
						// 人数-1
						CacheUtil.STREAMMAP.get(tokens).setCount(CacheUtil.STREAMMAP.get(tokens).getCount() - 1);
						logger.info("关闭：" + CacheUtil.STREAMMAP.get(tokens).getRtsp() + ";当前使用人数为："
								+ CacheUtil.STREAMMAP.get(tokens).getCount());
					}
				}
			}
		}
